...
icon_bluetooth_on="${icon_bluetooth_on:-"󰂯"}"
icon_bluetooth_off="${icon_bluetooth_off:-"󰂲"}"
icon_bluetooth_connected="${icon_bluetooth_connected:-"󰂱"}"
icon_bluetooth_device="${icon_bluetooth_device:-"󰥰"}"
icon_bluetooth_headphone="${icon_bluetooth_headphone:-"󰋋"}"
icon_bluetooth_speaker="${icon_bluetooth_speaker:-"󰓃"}"
icon_bluetooth_mouse="${icon_bluetooth_mouse:-"󰍽"}"
icon_bluetooth_keyboard="${icon_bluetooth_keyboard:-"󰌌"}"
icon_bluetooth_known="${icon_bluetooth_known:-"󰂴"}"
...
# Bluetooth
tr_bluetooth_message='Bluetooth'
tr_bluetooth_enable_message='Enable Bluetooth'
tr_bluetooth_disable_message='Disable Bluetooth'
tr_bluetooth_scan_message='Scan for devices'
tr_bluetooth_scanning='Scanning for devices...'
tr_bluetooth_no_devices='No Bluetooth devices found.'
tr_bluetooth_paired_devices='Paired Devices'
tr_bluetooth_available_devices='Available Devices'
tr_bluetooth_known_devices_message='Known Devices'
tr_bluetooth_no_paired_devices='No paired devices.'
tr_bluetooth_connecting='Connecting to device...'
tr_bluetooth_disconnecting='Disconnecting from device...'
tr_bluetooth_pairing='Pairing with device...'
tr_bluetooth_removing='Removing device...'
tr_bluetooth_pair_message='Pair'
tr_bluetooth_unpair_message='Unpair'
tr_bluetooth_trust_message='Trust'
tr_bluetooth_untrust_message='Untrust'
tr_bluetooth_connect_message='Connect'
tr_bluetooth_disconnect_message='Disconnect'
tr_bluetooth_remove_message='Remove Device'
...
tr_bluetooth_menu_prompt="$icon_bluetooth Bluetooth: $icon_search"
...
send_notification() {
	local summary="$1"
	local body="$2"
	local icon="${3:-}" # Optional third parameter for custom icon

	# Auto-detect icon based on context if not provided
	if [ -z "$icon" ]; then
		# Check body first for more accurate detection
		if [[ "$body" =~ Bluetooth ]] || [[ "$body" =~ bluetooth ]] || [[ "$body" =~ Paired ]] || [[ "$body" =~ paired ]] || [[ "$summary" =~ Bluetooth ]] || [[ "$summary" =~ Paired ]] || [[ "$summary" =~ Device\ Removed ]]; then
			icon="bluetooth"
		elif [[ "$summary" =~ VPN ]] || [[ "$body" =~ VPN ]]; then
			icon="network-vpn"
		elif [[ "$summary" =~ Wired ]] || [[ "$summary" =~ Ethernet ]]; then
			icon="network-wired"
		else
			# Default to network-wireless for all other cases
	...
	if command -v notify-send &>/dev/null; then
		notify-send -a "HyprLTM-Net" -i "$icon" "$summary" "$body"
	else
...
toggle_bluetooth() {
	if is_bluetooth_enabled; then
		...
		send_notification "$tr_bluetooth_message" "$tr_bluetooth_disable_message" "bluetooth"
	else
		bluetoothctl power on
		send_notification "$tr_bluetooth_message" "$tr_bluetooth_enable_message" "bluetooth"
	fi
}

# Get device icon based on device type
get_bluetooth_device_icon() {
	local device_info="$1"

	case "$device_info" in
	*"audio-card"* | *"audio-headset"* | *"audio-headphones"*)
		echo "$icon_bluetooth_headphone"
		;;
	*"audio-speakers"*)
		echo "$icon_bluetooth_speaker"
		;;
	*"input-mouse"*)
		echo "$icon_bluetooth_mouse"
		;;
	*"input-keyboard"*)
		echo "$icon_bluetooth_keyboard"
		;;
	*)
		echo "$icon_bluetooth_device"
		;;
	esac
}
...
scan_bluetooth_devices() {
	show_loading_notification "$tr_bluetooth_scanning"

	# Start scanning - 15s gives time for name resolution
	# We keep scan on in background so devices remain available for pairing
...
get_paired_devices() {
	bluetoothctl devices Paired 2>/dev/null | while read -r line; do
		...
		if [ -n "$mac" ]; then
			...
			local connected=$(echo "$info" | grep -q "Connected: yes" && echo "yes" || echo "no")

			# Resolve the best name from bluetoothctl info
			local resolved_name=$(echo "$info" | grep -m1 "^\s*Alias:" | sed 's/^\s*Alias:\s*//')
			if [ -z "$resolved_name" ] || echo "$resolved_name" | grep -qE '^([0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}$'; then
				resolved_name=$(echo "$info" | grep -m1 "^\s*Name:" | sed 's/^\s*Name:\s*//')
			fi
			if [ -n "$resolved_name" ] && ! echo "$resolved_name" | grep -qE '^([0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}$'; then
				name="$resolved_name"
			fi

			local icon=$(get_bluetooth_device_icon "$info")

			if [ "$connected" = "yes" ]; then
				echo "$mac|$name|connected|$icon"
			else
				echo "$mac|$name|paired|$icon"
			fi
		fi
	done
}

# Get list of available (discovered) devices
	...
	bluetoothctl devices 2>/dev/null | while read -r line; do
		...
		if [ -n "$mac" ]; then
			...
			if ! bluetoothctl devices Paired 2>/dev/null | grep -q "$mac"; then
				...
				# Try to resolve the real name from bluetoothctl info
				# (Alias is the user-friendly name, Name is the device-reported name)
				local resolved_name=$(echo "$info" | grep -m1 "^\s*Alias:" | sed 's/^\s*Alias:\s*//')
				if [ -z "$resolved_name" ] || echo "$resolved_name" | grep -qE '^([0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}$'; then
					resolved_name=$(echo "$info" | grep -m1 "^\s*Name:" | sed 's/^\s*Name:\s*//')
				fi

				# Skip devices that still only have a MAC address as name
				if [ -n "$resolved_name" ] && ! echo "$resolved_name" | grep -qE '^([0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}$'; then
					name="$resolved_name"
				else
					# If no real name resolved, skip the device entirely
					continue
				fi

				local icon=$(get_bluetooth_device_icon "$info")
				echo "$mac|$name|available|$icon"
			fi
		fi
	done
}
...
connect_bluetooth_device() {
	...
	show_loading_notification "$tr_bluetooth_connecting"

	if bluetoothctl connect "$mac" 2>/dev/null; then
		kill_loading_notification
		send_notification "$tr_notice_connected_summary" "$tr_notice_connected_body '$name'" "bluetooth"
	else
		kill_loading_notification
		send_notification "$tr_notice_error_summary" "$tr_notice_error_body '$name'" "bluetooth"
	fi
}
...
disconnect_bluetooth_device() {
	...
	show_loading_notification "$tr_bluetooth_disconnecting"

	if bluetoothctl disconnect "$mac" 2>/dev/null; then
		kill_loading_notification
		send_notification "$tr_notice_disconnected_summary" "$tr_notice_disconnected_body '$name'" "bluetooth"
	else
		kill_loading_notification
		send_notification "$tr_notice_error_summary" "Failed to disconnect from '$name'" "bluetooth"
	fi
}
...
pair_bluetooth_device() {
	local mac="$1"
	local name="$2"
	local pair_output_file
	pair_output_file=$(mktemp /tmp/bt_pair_XXXXXX)

	show_loading_notification "$tr_bluetooth_pairing"

	# Run bluetoothctl with agent in a subshell and capture output
	# The agent KeyboardDisplay allows passkey display + keyboard confirmation
	(
		echo "agent on"
		echo "default-agent"
		sleep 0.5
		echo "pair $mac"
		# Wait long enough for pairing to complete or fail
		sleep 30
	) | bluetoothctl 2>&1 | tee "$pair_output_file" &
	local bt_pid=$!

	# Monitor the output file for passkey or completion
	local timeout=25
	local elapsed=0
	local passkey_shown=false

	while [ $elapsed -lt $timeout ]; do
		sleep 1
		elapsed=$((elapsed + 1))

		# Check if a passkey/PIN is displayed
		if [ "$passkey_shown" = false ]; then
			local passkey=$(grep -oP 'Passkey:\s*\K[0-9]+' "$pair_output_file" 2>/dev/null)
			if [ -z "$passkey" ]; then
				passkey=$(grep -oP 'Enter passkey.*:\s*\K[0-9]+' "$pair_output_file" 2>/dev/null)
			fi
			if [ -n "$passkey" ]; then
				passkey_shown=true
				kill_loading_notification
				send_notification "$tr_bluetooth_pairing" "Type $passkey on '$name' then press Enter" "bluetooth"
			fi
		fi

		# Check if pairing succeeded
		if grep -q "Pairing successful" "$pair_output_file" 2>/dev/null; then
			kill "$bt_pid" 2>/dev/null
			wait "$bt_pid" 2>/dev/null
			bluetoothctl trust "$mac" &>/dev/null
			kill_loading_notification
			rm -f "$pair_output_file"
			send_notification "Paired" "Successfully paired with '$name'" "bluetooth"
			return
		fi

		# Check if pairing failed
		if grep -q "Failed to pair\|org.bluez.Error" "$pair_output_file" 2>/dev/null; then
			kill "$bt_pid" 2>/dev/null
			wait "$bt_pid" 2>/dev/null
			kill_loading_notification
			rm -f "$pair_output_file"
			send_notification "$tr_notice_error_summary" "Failed to pair with '$name'" "bluetooth"
			return
		fi
	...
	# Timeout or process ended without clear result
	kill "$bt_pid" 2>/dev/null
	wait "$bt_pid" 2>/dev/null
	kill_loading_notification
...
remove_bluetooth_device() {
	...
	show_loading_notification "$tr_bluetooth_removing"

	if bluetoothctl remove "$mac" 2>/dev/null; then
		kill_loading_notification
		send_notification "Device Removed" "Removed '$name'" "bluetooth"
	else
		kill_loading_notification
		send_notification "$tr_notice_error_summary" "Failed to remove '$name'" "bluetooth"
	fi
}
...
manage_bluetooth_device() {
	...
	while true; do
		...
		if [ "$status" = "connected" ]; then
			options+="$icon_disconnect  $tr_bluetooth_disconnect_message\n"
		elif [ "$status" = "paired" ]; then
			options+="$icon_connect  $tr_bluetooth_connect_message\n"
		else
			options+="$icon_connect  $tr_bluetooth_pair_message\n"
		fi

		if [ "$status" != "available" ]; then
			options+="$icon_trash  $tr_bluetooth_remove_message\n"
		fi

		options+="$icon_close Back"

		local choice=$(echo -e "$options" | display_menu 1 "$name" "")
		...
		case "$choice" in
		*"$tr_bluetooth_connect_message"*)
			connect_bluetooth_device "$mac" "$name"
			return
			;;
		*"$tr_bluetooth_disconnect_message"*)
			disconnect_bluetooth_device "$mac" "$name"
			return
			;;
		*"$tr_bluetooth_pair_message"*)
			pair_bluetooth_device "$mac" "$name"
			return
			;;
		*"$tr_bluetooth_remove_message"*)
			remove_bluetooth_device "$mac" "$name"
			return
			;;
		esac
	done
...
bluetooth_devices_menu() {
	while true; do
		...
		# Get paired devices
		while IFS='|' read -r mac name status icon; do
			if [ -n "$mac" ]; then
				has_paired=true
				local status_icon=""
				if [ "$status" = "connected" ]; then
					status_icon="$icon_bluetooth_connected"
				else
					status_icon="$icon_bluetooth_off"
				fi
				options+="$status_icon $icon  $name\n"
				devices+=("$mac|$name|$status")
			fi
		...
		while IFS='|' read -r mac name status icon; do
			if [ -n "$mac" ]; then
				has_available=true
				available_options+="$icon_bluetooth_off $icon  $name\n"
				devices+=("$mac|$name|$status")
			fi
		done < <(get_available_devices)

		# Add separator only if we have both paired AND available devices
		if [ "$has_paired" = true ] && [ "$has_available" = true ]; then
			options+="\n"
		fi
		...
		if [ -z "$options" ]; then
			display_info_message "$tr_bluetooth_no_devices" "$tr_bluetooth_message"
			return
		fi

		options+="$icon_close Back"

		local chosen=$(echo -e "$options" | display_menu 1 "$tr_bluetooth_message" "$icon_search")

		if [ -z "$chosen" ] || [[ "$chosen" =~ ^"$icon_close Back" ]]; then
			return
		fi

		# Extract device name from choice (remove icons)
		local chosen_name=$(echo "$chosen" | sed -E 's/^[^ ]+ [^ ]+  //')
		...
		for device in "${devices[@]}"; do
			...
			if [ "$name" = "$chosen_name" ]; then
				manage_bluetooth_device "$mac" "$name" "$status"
				break
			fi
		done
	done
...
# Menu for known/paired devices only
bluetooth_known_devices_menu() {
	while true; do
		local options=""
		local devices=()
		...
		if [ -z "$options" ]; then
			display_info_message "$tr_bluetooth_no_paired_devices" "$tr_bluetooth_known_devices_message"
			return
		fi

		options+="$icon_close Back"

		local chosen=$(echo -e "$options" | display_menu 1 "$tr_bluetooth_known_devices_message" "$icon_search")
...
bluetooth_menu() {
	while true; do
		local bt_status=""
		if is_bluetooth_enabled; then
			bt_status="$icon_bluetooth_on  $tr_bluetooth_disable_message"
		else
			bt_status="$icon_bluetooth_off  $tr_bluetooth_enable_message"
		fi

		local options="$bt_status\n"

		if is_bluetooth_enabled; then
			options+="$icon_bluetooth_known  $tr_bluetooth_known_devices_message\n"
			options+="$icon_bluetooth_device  $tr_bluetooth_paired_devices\n"
			options+="$icon_refresh  $tr_bluetooth_scan_message\n"
		fi

		options+="$icon_close Back"

		local choice=$(echo -e "$options" | display_menu 1 "$tr_bluetooth_menu_prompt" "")
		...
		case "$choice" in
		*"$tr_bluetooth_enable_message"* | *"$tr_bluetooth_disable_message"*)
			toggle_bluetooth
			;;
		*"$tr_bluetooth_known_devices_message"*)
			bluetooth_known_devices_menu
			;;
		*"$tr_bluetooth_paired_devices"*)
			bluetooth_devices_menu
			;;
		*"$tr_bluetooth_scan_message"*)
			scan_bluetooth_devices
			bluetooth_devices_menu
			;;
		esac
...
main_menu() {
	...
	options+="$icon_bluetooth  Bluetooth\n"
	...
	while true; do                                                                 # Keep looping for the main menu
		...
		*"Wired")
			...
			;;
		*"Bluetooth")
			bluetooth_menu # Call the Bluetooth sub-menu
			;;
		*"VPN")
			vpn_menu
			;;
